...
variable "s3_bucket_policy_not_more_permissive_parameters" {
  ...
  type = object({
    controlPolicy = optional(string, '{"Version":"2012-10-17","Statement":[]}')
  })
  default = {
    controlPolicy =  '{"Version":"2012-10-17","Statement":[]}'
  }
}
...
variable "s3_lifecycle_policy_check_parameters" {
  description = "targetTransitionDays(30), targetExpirationDays(90/99999), targetTransitionStorageClass("STANDARD_IA"), targetPrefix(optional_string), bucketNames(optional_string). Input parameters for the s3-lifecycle-policy-check rule."
  type = object({
