...
def _str_representer(dumper: yaml.Dumper, data: str) -> yaml.nodes.ScalarNode:
    # Keep plain strings as plain scalars when possible
...
def build_sot_index(sot: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
...
def build_pack_template(rule_names: List[str], sot_index: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate AWS Config Conformance Pack YAML files from a list of managed rule names and a Source of Truth file."
    )
    parser.add_argument(
        "-t",
        "--truth-file",
